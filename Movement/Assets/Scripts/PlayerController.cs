...
public class PlayerController : MonoBehaviour
{
    ...
    public delegate void LockOnEventHandler(object sender,LockOnEventArgs args);
    public event LockOnEventHandler LockOnEvent;
    public event EventHandler LockOffEvent;
    public class LockOnEventArgs : EventArgs
    {
        public LockOnEventArgs(Transform e)
        {
            Enemy = e;
        }
    ...
    private bool isItHighTime = false, IsNotLooking = false, isAirborn;
    ...
    void Start()
    {
        ...
        SwordRaveDelegate SRD = new SwordRaveDelegate(SwordRave);
        ...
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.still, "Attack1", false), SRD);
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.still, "Attack2", true), SaD);
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.still, "Attack2", false), ARTD);

        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.forward, "Attack1", true), SD);
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.forward, "Attack1", false), SRD);
        ...
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.back, "Attack1", false), HBD);
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.back, "Attack2", true), RAD);
        attackDictionary[0].Add(new Tuple<attackStates, string, bool>(attackStates.back, "Attack2", false), ARTD);
    ...
    void Update()
    {
        ...
        switch (state)
        {
            ...
            case states.running:
                ...
                if (playerInput.Player.LockOn.IsPressed() && dirState == attackStates.back) move *= speed * 0.5f;
                else move *= speed;

                break;

        }
        if (playerInput.Player.LockOn.IsPressed() && lockTarget != transform)
        {
        ...
        {
            LockOffEvent?.Invoke(this, EventArgs.Empty);
    ...
    private void checkAttack(InputAction.CallbackContext context)
    {
        ...
        if (context.action.name == "Attack2" && !playerInput.Player.LockOn.IsPressed()) return;
        state = states.attacking;
    ...
    private void Lock(InputAction.CallbackContext context)
    {
        ...
        {
            RaycastHit hit;
            if (Physics.Raycast(transform.position, enemies[i].transform.position - transform.position, out hit, 20f))
            {
        ...
        {
            yield return new WaitForEndOfFrame();
            LockOnEventArgs args = new(lockTarget);
            LockOnEvent?.Invoke(this, args);
        }
